...
function removeExifFromFile(baseFile) {
    if (baseFile == undefined) {
      return undefined;
    };
    const byteArray = new Uint8Array(await baseFile.arrayBuffer());
  ...
  const isJPEG = (byteArr) => {
    // the first two byte pairs in a jpeg are ffd8 in hexadecimal; SOI (Start of Image) Marker
    const beginningBytePair = byteArr[0].toString(16) + byteArr[1].toString(16);
  ...
  const handleJPEG = (byteArray, baseFile) => {
    let lastIndex = 0;
    let piecesToKeep = [];
    ...
    const byteArraySlices = piecesToKeep.map((pair) => {
    ...
    const newFile = new File(newByteArray, baseFile.name, {type:baseFile.type});
  
    return newFile;
  }
  
  module.exports = removeExifFromFile;